...
function addClaims(parameters, correlationId, claims, clientCapabilities, skipBrokerClaims) {
	const mergedClaims = buildMergedClaims$1(claims, skipBrokerClaims && parameters.has("brk_client_id") ? void 0 : clientCapabilities, correlationId);
...
function buildClientConfiguration({ authOptions: userAuthOptions, systemOptions: userSystemOptions, loggerOptions: userLoggerOption, storageInterface: storageImplementation, networkInterface: networkImplementation, cryptoInterface: cryptoImplementation, clientCredentials, libraryInfo, telemetry, serverTelemetryManager, persistencePlugin, serializableCache }) {
	...
	return {
		...
		loggerOptions,
		storageInterface: storageImplementation || new DefaultStorageClass(userAuthOptions.clientId, DEFAULT_CRYPTO_IMPLEMENTATION, new Logger(loggerOptions, "@azure/msal-common", "16.11.1"), new StubPerformanceClient()),
...
var BrowserCacheManager = class extends CacheManager {
	...
	*/
	trackVersionChanges(correlationId) {
		...
		if (previousVersion !== "5.17.0") this.setItem(VERSION_CACHE_KEY, version, correlationId);
	}
	/**
	...
	*/
	...
	setInteractionInProgress(inProgress, type = INTERACTION_TYPE.SIGNIN, allowOverride = false, correlationId = "") {
		...
		if (inProgress) {
			...
			if (existingInteraction) if (allowOverride) {
...
var EventHandler = class {
	constructor(logger) {
		...
		this.logger = logger || new Logger({}, "@azure/msal-browser", "5.17.0");
...
var PlatformAuthInteractionClient = class extends BaseInteractionClient {
	...
	*/
	async cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, tenantId, reqTimestamp, environment) {
		...
		const cachedAccessToken = createAccessTokenEntity(homeAccountIdentifier, environment, response.access_token, request.clientId, idTokenClaims.tid || tenantId, responseScopes.printScopes(), tokenExpirationSeconds, 0, base64Decode, request.correlationId, void 0, request.tokenType, void 0, request.keyId);
	...
	onChannelMessage(event) {
		...
		try {
			...
			if (method === NativeExtensionMethod.Response) {
				...
				else if (response.result) if (response.result["code"] && response.result["description"]) resolver.reject(createNativeAuthError(response.result["code"], correlationId, response.result["description"], response.result["ext"]));
	...
	*/
	async hydrateCache(result, request) {
		...
		if (result.fromPlatformBroker) {
			...
			const accessTokenEntity = createAccessTokenEntity(result.account.homeAccountId, result.account.environment, result.accessToken, this.config.auth.clientId, result.tenantId, result.scopes.join(" "), result.expiresOn ? toSecondsFromDate(result.expiresOn) : 0, result.extExpiresOn ? toSecondsFromDate(result.extExpiresOn) : 0, base64Decode, request.correlationId || "", void 0, result.tokenType, void 0, request.sshKid);
			if (request.resource) accessTokenEntity.resource = request.resource;
...
//#endregion
export { ApiId, AuthError, AuthErrorCodes_exports as AuthErrorCodes, AuthenticationHeaderParser, AuthenticationScheme, AzureCloudInstance, BrowserAuthError, BrowserAuthErrorCodes_exports as BrowserAuthErrorCodes, BrowserCacheLocation, BrowserConfigurationAuthError, BrowserConfigurationAuthErrorCodes_exports as BrowserConfigurationAuthErrorCodes, BrowserPerformanceClient, BrowserPerformanceMeasurement, BrowserRootPerformanceEvents_exports as BrowserRootPerformanceEvents, BrowserUtils_exports as BrowserUtils, CacheLookupPolicy, ClientAuthError, ClientAuthErrorCodes_exports as ClientAuthErrorCodes, ClientConfigurationError, ClientConfigurationErrorCodes_exports as ClientConfigurationErrorCodes, DEFAULT_IFRAME_TIMEOUT_MS, EventHandler, EventMessageUtils, EventType, InteractionRequiredAuthError, InteractionRequiredAuthErrorCodes_exports as InteractionRequiredAuthErrorCodes, InteractionStatus, InteractionType, JsonWebTokenTypes, LocalStorage, LogLevel, Logger, MemoryStorage, NavigationClient, OIDC_DEFAULT_SCOPES, PromptValue, ProtocolMode, PublicClientApplication, ResponseMode, ServerError, SessionStorage, SignedHttpRequest, StubPerformanceClient, WrapperSKU, createNestablePublicClientApplication, createStandardPublicClientApplication, enforceResourceParameter, isPlatformBrokerAvailable, loadExternalTokens, stubbedPublicClientApplication, version };
